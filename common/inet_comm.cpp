...
*/
    ...
    LOG_ERR("socket side must be CLIENT or SERVER");
  }
}
...
int socket_read(
  ...
  LOG_INFO("Reading %d bytes: %s", _num_bytes, buf);
  */
...
int socket_write(
  ...
  LOG_INFO("Writing %d bytes: %s", _num_bytes, buf);
  */
...
int socket_loop(
  ...
  if (sock_side == SERVER) {
    LOG_INFO("Opening socket as server");
  } else if (sock_side == CLIENT) {
    LOG_INFO("Opening socket as client");
  } else {
    LOG_ERR("Socket side must be either `client` or `server`");
    return ELKA_ERROR;
  }
...
int socket_read_elka_msg(
  ...
  if ( (num_bytes = socket_read(
    ...
    LOG_INFO("performing socket read cb"); fflush(stdout);
